...
function get {
  ...
  # Copy|download HiveOS image to tmpfs
  echo -e "${GREEN}\nGet HiveOS image${NOCOLOR}"
  if [[ "$url" == *http* ||  "$url" == *ftp* ]]; then
    echo ""
    wget -t 5 --show-progress $url -O $archname
  ...
  [[ $? -ne 0 ]] && "${RED}Can't get the required data from Hive OS server. Check your internet connection${NOCOLOR}" && return 1
  archname="hiveos-0.6-222-stable@230512.img.xz"  # Hardcoded XZ image filename
  mdsum=$(sed -n "/$archname/p" $TMPDIR/VERSIONS.txt | awk '{print$2}')
  
  # Check md5sum
  echo "${GREEN}Check MD5 sum of image file${NOCOLOR}"
  mdsum_hive=$(md5sum $archname | awk '{print$1}')
  [[ $mdsum != $md5_hive ]] && echo "${RED}MD5 sum of image file incorrect${NOCOLOR}" && return 1
